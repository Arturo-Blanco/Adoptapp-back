...
export class PetsService {
  ...
  async addPet(petDTO: CreatePetDTO): Promise<string> {
    ...
    try {
      ...
      if (!checkValues(petDTO, this.validValues)) {
        throw new Error('Required fields missing: name, specie, age, description, attributes, urlImg, institution_id.');
      ...
      newPet.fk_institution_id = institution_id;
  ...
  // funtion to get pet with id
  async findById(petId: number): Promise<Pet> {
    try {
      const criteria: FindOneOptions = { where: { id: petId } }
      const pet = await this.petRepository.findOne(criteria);
      if (!pet) {
        throw new NotFoundException('There is not pet.');
      }
      return pet;
    } catch (error) {
      throw new HttpException({
  ...
  async filterPets(pageNumber: number, specie?: string, location_id?: number, sex?: string): Promise<PetDTO[]> {
    ...
    try {
      ...
      const criterion: FindManyOptions = {
        relations: ['attributes', 'institution', 'institution.city'],
        take: elementsPage,
        where: {
          available: true,
          ...filter,
      ...
      const petData: PetDTO[] = leakedPets.map(pet => ({
        ...
        city: pet.institution.city.name
      }));
      return petData;
  ...
  async allPets(): Promise<Pet[]> {
    try {
      ...
      const criterion: FindManyOptions = { relations: ['attributes'] };
      const data = await this.petRepository.find(criterion);

      if (data.length === 0) {
        throw new NotFoundException('There is no pets on the database.')
      }
      if (!data) {
  ...
  async updatePet(petId: number, body: UpdatePetDTO): Promise<string> {
    const { name, age, sex, specie, attributes, description, urlImg } = body;
    try {
      const criterion: FindOneOptions = { where: { id: petId } };
      const pet: Pet = await this.petRepository.findOne(criterion);
      if (!pet) {
      ...
      if (!checkValues(body, this.validValues)) {
        throw new Error(`Invalid key for the pet. Please enter name, sex, age, description, image url, institution_id or species.`)
      }
